...
use harnspec_core::sessions::RunnerRegistry;
...
fn run_standard_init(specs_dir: &str, options: InitOptions) -> Result<(), Box<dyn Error>> {
    ...
    if specs_path.exists() && specs_path.is_dir() {
        ...
        if !options.yes && readme_exists {
            println!(
                "{}",
                "HarnSpec already initialized in this directory.".yellow()
            );
            println!(
                "Specs directory: {}",
    ...
    println!();
    println!("{}", "HarnSpec initialized successfully! 🎉".green().bold());
    println!();
    println!("Next steps:");
    println!(
...
This directory contains HarnSpec specifications for this project.
...
fn decide_skill_install(options: &InitOptions) -> Result<bool, Box<dyn Error>> {
    ...
    let confirm = Confirm::new()
        .with_prompt("Install HarnSpec agent skills? (recommended)")
        .default(true)
        .interact()?;
