...
use harnspec_core::storage::config::config_dir;
...
async fn build_manager() -> Result<SessionManager, Box<dyn Error>> {
    ...
    let unified_db_path = sessions_dir.join("harnspec.db");
    let database = harnspec_core::db::Database::connect(&unified_db_path)
        .await
...
async fn merge_worktree_record(
    manager: &SessionManager,
    session: &mut harnspec_core::sessions::Session,
