...
//!
//! When `HARNSPEC_API_KEY` is set, all `/api/*` requests must include
...
/// Middleware that enforces bearer-token authentication when `HARNSPEC_API_KEY`
...
pub async fn api_key_auth(req: Request<Body>, next: Next) -> Response {
    let expected = match std::env::var("HARNSPEC_API_KEY") {
