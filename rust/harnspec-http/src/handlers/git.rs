...
use harnspec_core::git::{CloneManager, RemoteRef, SpecDetectionResult};
...
fn clone_dir_for(remote_url: &str) -> std::path::PathBuf {
    ...
    let slug: String = remote_url
        ...
        .collect();
    harnspec_core::storage::config::config_dir()
        .join("repos")
        .join(slug)
}
...
/// POST /api/git/import — Clone a Git repo and register it as a HarnSpec project.
pub async fn git_import_repo(
    ...
    if !CloneManager::is_valid_clone(&clone_dir) {
        tokio::task::spawn_blocking(move || {
            let config = harnspec_core::git::CloneConfig {
...
pub async fn git_push_project(
    ...
    let message = body
        .message
        .unwrap_or_else(|| "Update specs via HarnSpec".to_string());
...
pub async fn git_status_project(
    State(state): State<AppState>,
    axum::extract::Path(project_id): axum::extract::Path<String>,
) -> Result<Json<harnspec_core::git::GitStatus>, (StatusCode, String)> {
    let registry = state.registry.read().await;
    let project = registry
        .get(&project_id)
        .ok_or_else(|| (StatusCode::NOT_FOUND, "Project not found".to_string()))?;
