...
# Brand Reconstruction: CoSpec

## Overview

LeanSpec is being rebranded to **CoSpec** to reflect a new focus on **Collaborative Specification and Harness-Driven Development** for small teams (2-10 people). This rebranding involves updating all package names, CLI commands, and documentation. The project will now be hosted on GitHub Pages (`https://codervisor.github.io/cospec/`) to maintain a lightweight and community-driven presence.
...
### 2. Package Updates
- Root project renamed to `cospec`.
- CLI package renamed to `cospec`.
- All `@leanspec/*` packages renamed to `@cospec/*`.

### 3. Documentation
- Update all instances of "LeanSpec" to "CoSpec" in the documentation site.
- Move documentation hosting to GitHub Pages (`https://codervisor.github.io/cospec/`).
...
- Rename `bin/lean-spec.js` to `bin/cospec.js`.

### Rust Components
- Rename crates from `leanspec-*` to `cospec-*`.
- Update directory names in the `rust/` folder.

## Verification

### Acceptance Criteria
- [ ] `cospec --help` works correctly with the new name.
- [ ] All packages publishable under `@cospec` scope.
- [ ] Documentation site correctly displays "CoSpec" branding.
