...
Teach agents how to run Spec-Driven Development (SDD) in HarnSpec projects using the `harnspec` CLI.
...
- Run `harnspec view <spec>` — includes parent, children, depends_on, required_by
- Set status to `in-progress`: `harnspec update <spec> --status in-progress`
...
- Status: `harnspec update <spec> --status <status>`
- Dependencies: `harnspec rel add <spec> --depends-on <other>`
- Parent: `harnspec rel add <child> --parent <parent>`
...
All operations use the `harnspec` CLI. Run commands via shell/Bash.
...
| Add child | `harnspec rel add <parent> --child <child>` |
| Add dependency | `harnspec rel add <spec> --depends-on <other>` |
| Remove dependency | `harnspec rel rm <spec> --depends-on <other>` |
| Dependency graph | `harnspec deps <spec>` |
| List children | `harnspec children <parent>` |
| Token count | `harnspec tokens <spec>` |
| Validate | `harnspec validate` |
| Stats | `harnspec stats` |
...
**Command**: `harnspec rel add <child> --parent <parent>`
...
**Command**: `harnspec rel add <spec> --depends-on <other>`
**Remove**: `harnspec rel rm <spec> --depends-on <other>`
...
1. Create the umbrella spec: `harnspec create <name>`
2. Create each child spec: `harnspec create <name>`
3. Assign children: `harnspec rel add <child> --parent <parent>` for each child
4. Verify structure: `harnspec children <parent>`
5. Add cross-cutting deps if needed: `harnspec rel add <spec> --depends-on <other>`
