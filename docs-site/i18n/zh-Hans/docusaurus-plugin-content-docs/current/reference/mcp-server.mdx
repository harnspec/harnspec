...
将 `/path/to/your/lean-spec/project` 替换为您的 HarnSpec 项目目录的绝对路径。
...
如果您全局安装了 HarnSpec（`npm install -g lean-spec`），可以使用：
...
- 验证配置中的 `cwd` 路径指向有效的 HarnSpec 项目（有 `.lean-spec/config.json`）
...
- 配置更改后重新启动 MCP 客户端
- 检查客户端日志是否有连接错误
- 验证 HarnSpec 项目是否正确初始化（`lean-spec init`）
