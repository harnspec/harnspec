---
id: ui-package
title: '@HarnSpec/ui Package'
sidebar_label: '@HarnSpec/ui'
sidebar_position: 4
description: Reference documentation for the @HarnSpec/ui npm package
---

# @HarnSpec/ui Package

The `@HarnSpec/ui` package is a standalone web UI for HarnSpec that can be run without installing the CLI.

## Overview

`@HarnSpec/ui` is a Next.js-based web application that provides a visual interface for browsing and managing HarnSpec specifications. It's published to npm and can be run directly with `npx` or installed globally.
...
```bash
npm install --save-dev @HarnSpec/ui
```
...
npx @HarnSpec/ui --specs ./documentation/specs

# Custom port
npx @HarnSpec/ui --port 8080

# Don't open browser
npx @HarnSpec/ui --no-open

# Combine options
npx @HarnSpec/ui --specs ./specs --port 3100 --no-open
```
...
```mermaid
graph TD
    CLI[harnspec ui] -->|Delegates| NPX[npx @HarnSpec/ui]
...
If you're contributing to HarnSpec, you can work with the UI package locally:

```bash
# Clone the HarnSpec monorepo
git clone https://github.com/codervisor/harnspec.git
cd harnspec
...
```bash
# Specify explicitly
npx @HarnSpec/ui --specs ./path/to/specs
...
npx @HarnSpec/ui 2>&1 | tee ui-error.log
```

## Comparison with CLI

| Feature | `@HarnSpec/ui` | `harnspec ui` (CLI) |
|---------|----------------|---------------------|
...
- **Use `@HarnSpec/ui`** for quick viewing without installing anything
- **Use `harnspec ui`** if you already have the CLI and want consistent commands

Both launch the same web interface - the difference is how you start it.

## API Reference

While `@HarnSpec/ui` doesn't expose a programmatic API, you can import and use the Next.js server programmatically:

```javascript
// Not officially supported, but possible for advanced use cases
import { createServer } from '@HarnSpec/ui/.next/standalone/server.js';
...
The `@HarnSpec/ui` package follows semantic versioning:

- **Major**: Breaking changes to CLI interface or behavior
- **Minor**: New features, non-breaking changes
- **Patch**: Bug fixes, performance improvements

Check the [CHANGELOG](https://github.com/codervisor/harnspec/blob/main/CHANGELOG.md) for version details.

## Package Information

- **npm**: https://www.npmjs.com/package/@HarnSpec/ui
- **Repository**: https://github.com/codervisor/harnspec
- **Bug Reports**: https://github.com/codervisor/harnspec/issues
- **License**: MIT

## Related Documentation

- [Visual Mode Guide](/docs/guide/visual-mode) - Comprehensive usage guide
- [CLI Reference](/docs/reference/cli#harnspec-ui) - `harnspec ui` command
- [Getting Started](/docs/guide/getting-started) - Set up HarnSpec
- [Configuration](/docs/reference/config) - Configure HarnSpec settings
