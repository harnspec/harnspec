...
Complete reference for `.harnspec/config.json`.

## Configuration File

HarnSpec configuration is stored in `.harnspec/config.json` in your project root.
...
Path to the template file within `.harnspec/templates/` directory.
...
Allows you to define multiple templates that users can select from during `harnspec create` or `harnspec init`.
...
When enabled, HarnSpec automatically checks for sequence number conflicts during spec creation and warns if duplicates exist.
