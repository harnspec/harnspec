...
sidebar_position: 8
description: Using the HarnSpec web UI for visual spec management
---

# Visual Mode

The HarnSpec UI provides a visual web interface for browsing and managing specs. It's perfect for visual learners, team presentations, and exploratory spec browsing.

![HarnSpec UI list view showing spec cards with search and filters](/img/ui/ui-spec-list.png)
...
### Method 1: Using `harnspec ui`

If you have the HarnSpec CLI installed:
...
### Method 2: Using `npx @HarnSpec/ui`
...
- Sharing with team members who don't have HarnSpec installed
...
# Combine options
harnspec ui --specs ./docs/specs --port 3100 --no-open
```
...
```mermaid
flowchart LR
    CLI[harnspec ui] -- delegates --> Local[Local Next.js build]
    External[npx @HarnSpec/ui] -- downloads --> Package["@HarnSpec/ui package"]
...
harnspec create new-feature
harnspec update new-feature --status=active
...
harnspec ui --specs ./path/to/specs
...
**Solution**:
This only affects HarnSpec contributors in the monorepo:
```bash
cd packages/web
npm run build
```
...
- **Learn More**: See [CLI Reference](/docs/reference/cli#harnspec-ui) for all command options
- **UI Package**: Check out [@HarnSpec/ui](/docs/reference/ui-package) for technical details
- **Get Started**: Read [Getting Started](/docs/guide/getting-started) to set up your first specs

## Related Documentation

- [CLI Reference - `harnspec ui` command](/docs/reference/cli#harnspec-ui)
