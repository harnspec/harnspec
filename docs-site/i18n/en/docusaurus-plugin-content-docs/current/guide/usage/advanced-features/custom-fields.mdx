...
HarnSpec supports powerful customization through custom frontmatter fields and template variables.
...
**When updating specs:**
```bash
harnspec update 001 --field reviewer=alice --field estimate=large
```

**When filtering:**
```bash
harnspec list --field epic=PROJ-123
harnspec list --field sprint=42
harnspec search "API" --field team=backend
```
...
Create or edit `.harnspec/templates/spec-template.md`:
...
**With all options:**
```bash
harnspec create payment-integration \
...
harnspec update 001-user-authentication \
  --field sprint=6 \
  --field estimate=2d
...
```bash
# Filter by epic
harnspec list --field epic=AUTH-2024

# Filter by sprint
harnspec list --field sprint=5

# Combine with standard filters
harnspec list --status in-progress --field sprint=6

# Multiple custom fields
harnspec list --field epic=AUTH-2024 --field sprint=6

# Search with custom field filter
harnspec search "authentication" --field epic=AUTH-2024
```
...
HarnSpec validates types automatically. Use correct types to catch errors early.
:::
...
HarnSpec automatically coerces values to the correct type:

```bash
# "42" is converted to number 42
harnspec create feature --field sprint=42

# "true" is converted to boolean true
harnspec create feature --field needs_review=true

# Arrays work with comma separation
harnspec create feature --field teams=backend,frontend
```
...
harnspec update 001 --field epic=PROJ-123
