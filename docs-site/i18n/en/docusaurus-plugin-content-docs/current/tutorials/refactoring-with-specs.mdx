...
Not all specs are features. Learn how to use HarnSpec for refactoring and architectural changes where documentation is critical.
...
```bash
npx harnspec init --example api-refactor
cd api-refactor
npm install
npm start
```
...
```bash
harnspec update 024 --status complete
```
...
**Questions?** See [FAQ](/docs/faq) or [start a discussion](https://github.com/codervisor/harnspec/discussions).
