...
HarnSpec specs function as a **persistent memory layer** for AI agents—providing long-term knowledge storage that persists across sessions and enables effective human-AI collaboration.
...
**HarnSpec solution**: Specs as **semantic memory** - structured, persistent knowledge that agents can reliably access
...
- **Storage**: HarnSpec specs + CLI retrieval
- **Size**: Unlimited (external storage)

**HarnSpec specs = Semantic memory for AI agents**
...
1. **Agent Skills**: `AGENTS.md` and `SKILL.md` guide the agent to use CLI tools for retrieval
...
AI Agent:
1. Runs HarnSpec CLI: `harnspec search "database decision"`
...
```mermaid
flowchart LR
    Human[Human Intent] -->|Writes| Spec[HarnSpec Spec]
    Spec -->|CLI Retrieval| AI[AI Agent]
...
- **Storage**: HarnSpec specs (README, DESIGN, etc.)
...
**HarnSpec primarily provides semantic memory** - the persistent knowledge layer.
...
- Test CLI search retrieves relevant specs
...
- ✅ Use CLI search to retrieve before asking humans
...
**HarnSpec transforms specs from static documentation into active semantic memory for AI agents.**

- **Working memory** (chat history) is ephemeral and limited
- **Semantic memory** (specs) is persistent and scalable
- **Agent Skills** enable reliable memory retrieval
...
**Related**: Learn about [Context Engineering](/docs/advanced/context-engineering) for managing memory size, or explore [Agent Configuration](/docs/guide/usage/advanced-features/agent-configuration) for setup instructions.
