...
HarnSpec specs can be written by humans, AI, or co-created collaboratively. This guide covers how to work with AI to draft, refine, and maintain specs while keeping them lean and actionable.
...
Ask AI to draft a spec following HarnSpec principles:

```
Draft a HarnSpec for [your intent]. Follow these principles:
...
```
Review this spec against HarnSpec first principles:
...
- Use `harnspec create` or update frontmatter
...
3. Once approach is clear: "Summarize our conversation as a HarnSpec"
...
Explicitly remind AI to follow HarnSpec principles when drafting:

```
Draft this spec following HarnSpec principles:
...
Use `harnspec update <spec> --status <status>` to track progress. AI can help draft, but you manage lifecycle.
