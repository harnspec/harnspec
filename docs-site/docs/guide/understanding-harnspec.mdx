...
HarnSpec 的目标是让人类和 AI 都能在一个文件内理解工作意图。这一页解释结构背后的原因、如何随着复杂度升级结构，以及如何保持Spec长期有效。

## 核心原则

HarnSpec 建立在五个关键原则之上，旨在弥合人类意图与 AI 执行之间的差距：
...
- 合并前运行 `harnspec validate` 与 `harnspec tokens <spec>`，确保元数据完整、内容不过载。
- 定期检查链接（仪表板、CLI 命令、PR 等）是否仍然有效。
...
## 与 AI 协作
1. 提供Spec路径或让 Agent Skills 搜索去加载上下文。
...
- [使用 AI 完成第一个功能](/docs/tutorials/first-spec-with-ai) —— 把理念用在真实项目
