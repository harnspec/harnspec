...
sidebar_label: 可视化模式
sidebar_position: 8
description: 使用 HarnSpec 网页界面进行可视化Spec管理
---
...
![HarnSpec UI 列表视图，展示搜索和筛选控件](/img/ui/ui-spec-list.png)
...
```mermaid
flowchart LR
    CLI[harnspec ui] --> Local[本地 Next.js 构建]
    External[npx @HarnSpec/ui] --> Package["@HarnSpec/ui 包"]
...
- **了解更多**：查看 [CLI 参考](/docs/reference/cli#harnspec-ui) 以获取所有命令选项
- **UI 包**：查看 [@HarnSpec/ui](/docs/reference/ui-package) 以获取技术细节
- **入门**：阅读[入门指南](/docs/guide/getting-started)以设置您的第一个Spec

## 相关文档

- [CLI 参考 - `harnspec ui` 命令](/docs/reference/cli#harnspec-ui)
