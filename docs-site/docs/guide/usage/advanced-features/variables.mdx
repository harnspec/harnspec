...
在 `.harnspec/config.json` 中定义您自己的变量：
...
`.harnspec/templates/spec-template.md`：
...
**解决方案：** 检查 `.harnspec/config.json` 是否定义了变量。
...
**下一步**：了解[模板](/docs/guide/usage/advanced-features/templates)或探索[配置参考](/docs/reference/config)。
