...
理解 HarnSpec Spec的结构：frontmatter、必需部分、子 Spec文件，以及如何构建人类和 AI 都能理解的Spec。
...
**重要**：这些是系统管理的。使用 `harnspec update` 修改，永远不要手动编辑。
...
**手动编辑**：只有 `depends_on` 和 `related` 必须手动编辑（尚无 CLI 命令）。所有其他字段使用 `harnspec update`。

**另见**：[frontmatter参考](/docs/reference/frontmatter)
...
**另见**：[编写 AI 可执行的Spec](/docs/advanced/ai-assisted-spec-writing)

## 下一步

- [创建和管理Spec](/docs/guide/usage/cli/creating-managing) - 基本操作
- [模板](/docs/guide/usage/advanced-features/templates) - Spec模板
- [frontmatter参考](/docs/reference/frontmatter) - 完整元数据指南

---

**参考**：[CLI 文档](/docs/reference/cli) 获取命令详情
