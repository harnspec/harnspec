...
`.harnspec/config.json` 的完整参考。

## 配置文件

HarnSpec 配置存储在项目根目录的 `.harnspec/config.json` 中。
...
允许您定义多个模板，用户可以在 `harnspec create` 或 `harnspec init` 期间选择。
...
启用后，HarnSpec 在Spec创建期间自动检查序列号冲突，如果存在重复则发出警告。
...
- 模板文件必须存在于 `.harnspec/templates/` 中
- 如果缺少Spec目录，则创建
...
**下一步**：查看 [CLI 参考](/docs/reference/cli)或探索[frontmatter字段](/docs/reference/frontmatter)。
