...
Replace `/path/to/your/lean-spec/project` with the absolute path to your HarnSpec project directory.
...
If you have HarnSpec installed globally (`npm install -g lean-spec`), you can use:
...
- Verify the `cwd` path in your configuration points to a valid HarnSpec project (has `.lean-spec/config.json`)
...
- Verify the HarnSpec project is properly initialized (`lean-spec init`)
