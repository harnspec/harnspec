...
sidebar_position: 4
description: '@HarnSpec/ui npm 包的参考文档'
---

# @HarnSpec/ui 包

`@HarnSpec/ui` 包是一个独立的 HarnSpec 网页 UI，无需安装 CLI 即可运行。

## 概述

`@HarnSpec/ui` 是一个基于 Next.js 的网页应用程序，为浏览和管理 HarnSpec Spec提供可视化界面。它发布在 npm 上，可以直接使用 `npx` 运行或全局安装。
...
```mermaid
graph TD
    CLI[harnspec ui] -->|委托| NPX[npx @HarnSpec/ui]
...
如果您正在为 HarnSpec 做贡献，可以本地使用 UI 包：

```bash
# 克隆 HarnSpec monorepo
...
| 功能 | `@HarnSpec/ui` | `harnspec ui`（CLI） |
|------|----------------|---------------------|
...
- **使用 `@HarnSpec/ui`** 用于无需安装任何内容的快速查看
- **使用 `harnspec ui`** 如果您已经有 CLI 并希望命令一致

两者都启动相同的网页界面 - 区别在于您如何启动它。

## 版本历史

`@HarnSpec/ui` 包遵循语义版本控制：

- **Major**：CLI 界面或行为的破坏性更改
- **Minor**：新功能、非破坏性更改
- **Patch**：错误修复、性能改进

查看 [CHANGELOG](https://github.com/codervisor/harnspec/blob/main/CHANGELOG.md) 了解版本详情。

## 包信息

- **npm**：https://www.npmjs.com/package/@HarnSpec/ui
- **仓库**：https://github.com/codervisor/harnspec
- **错误报告**：https://github.com/codervisor/harnspec/issues
- **许可证**：MIT

## 相关文档

- [可视化模式指南](/docs/guide/visual-mode) - 全面的使用指南
- [CLI 参考](/docs/reference/cli#harnspec-ui) - `harnspec ui` 命令
- [入门指南](/docs/guide/getting-started) - 设置 HarnSpec
- [配置](/docs/reference/config) - 配置 HarnSpec 设置
