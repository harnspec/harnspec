...
- **Clippy Errors** — Fixed new `unnecessary-sort-by` warnings introduced in newer Rust stable toolchains (1.95/1.96) by using `sort_by_key`.

## [0.2.2] - 2026-06-16

### Fixed

- **Clippy Errors** — Resolved Clippy warnings for `err-expect` in `harnspec-core` and parameters count limits (`too-many-arguments`) in `harnspec-cli` tests.
...
- **Node Requirement** — Downgraded minimum Node.js version from 24.0.0 to 22.0.0
...
- **Windows Runner Startup** — Resolved "program not found" error on Windows when spawning AI runners (node-based CLIs or `.ps1` scripts) by implementing absolute path resolution and optional PowerShell fallback.

### Changed

- **Gemini Runner** — Enabled `--yolo` flag for the builtin Gemini runner by default to support better autonomous coding sessions.
