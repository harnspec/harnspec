...
# Stops all running harnspec processes
...
PIDS=$(ps aux | grep -E "harnspec" | grep -v grep | grep -v "stop-harnspec-processes" | awk '{print $2}')
...
ps aux | grep -E "harnspec" | grep -v grep | grep -v "stop-harnspec-processes"
echo ""
...
REMAINING=$(ps aux | grep -E "harnspec" | grep -v grep | grep -v "stop-harnspec-processes" | wc -l)
